...
if(movement_percentage == 0){
	...
	initial_r_x = right_x
	initial_r_y = right_y
	delta_r_x = lengthdir_x(movement_length,90 - movement_angle )
	delta_r_y = lengthdir_y(movement_length,90 - movement_angle) 	
	delta_a_x = lengthdir_x(movement_length,facing - movement_angle)
	delta_a_y = lengthdir_y(movement_length,facing - movement_angle)
}

if(movement_percentage < first_checkpoint){
	right_x = initial_r_x - delta_r_x*speed_spreading *movement_percentage 
	right_y = initial_r_y - delta_r_y*speed_spreading *movement_percentage
}
...
if(movement_style == "normal" and movement_percentage >= first_checkpoint and movement_percentage < second_checkpoint){
	...
	right_x = initial_r_x - delta_r_x * (1 - speed_transfering*movement_remaining)
	right_y = initial_r_y - delta_r_y * (1 - speed_transfering*movement_remaining)
	x = initial_x + delta_a_x*speed_transfering*movement_remaining
	y = initial_y + delta_a_y*speed_transfering*movement_remaining
	left_x = initial_l_x + delta_r_x*speed_transfering*movement_remaining
	left_y = initial_l_y + delta_r_y*speed_transfering*movement_remaining
}
...
if(movement_percentage >= second_checkpoint and movement_percentage < third_checkpoint){
	...
	left_x = initial_l_x - delta_r_x * ( speed_spreading *movement_remaining -1)
	left_y = initial_l_y - delta_r_y * ( speed_spreading *movement_remaining -1)
}
